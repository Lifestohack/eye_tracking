...
def find_pupil_circle_marker(img, find_v2_marker=True,find_v3_marker=True):
    ...
    for i in range(len(cirlce_clusters)):
        ...
        if not detected and find_v3_marker:
            ...
            if detected:
                img_pos = cirlce_clusters[i][0][0]
                norm_pos = normalize(img_pos, img_size, flip_y=True)
                marker_list.append({'ellipse': cirlce_clusters[i], 'version': 3, 'img_pos': img_pos, 'norm_pos': norm_pos})
        for circle in marker_list:
            marker_dict = {}
