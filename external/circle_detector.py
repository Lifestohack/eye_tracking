...
import cv2
from external.methods import dist_pts_ellipse, normalize
...
if __name__ == "__main__":
    ...
    def bench():
        ...
        cap = cv2.VideoCapture("/home/luma/Documents/pupil/recordings/2020_12_12/001/world.mp4")
        cap.set(3, 1280)
        cap.set(4, 720)
        while (cap.isOpened()):
            ret, img = cap.read()
            if ret == True:
                ...
                pos = find_pupil_circle_marker(gray, scale=1)
                if len(pos) == 1:
                    first_circle = pos[0]['ellipses']
                    circle_center = first_circle[0][0]
                    x = int(circle_center[1])
                    y = int(circle_center[0])
                    radius = first_circle[-1][1][-1]
                    # Center coordinates
